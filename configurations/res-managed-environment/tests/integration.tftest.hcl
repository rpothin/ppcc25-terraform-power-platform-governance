...
variables {
  ...
  test_usage_insights_disabled = true
...
run "plan_validation" {
  ...
  variables {
    ...
    sharing_settings        = var.test_sharing_settings
    usage_insights_disabled = var.test_usage_insights_disabled
    solution_checker        = var.test_solution_checker
    maker_onboarding        = var.test_maker_onboarding
  }
  ...
  assert {
    condition     = powerplatform_managed_environment.this.is_group_sharing_disabled == var.test_sharing_settings.is_group_sharing_disabled
    error_message = "Group sharing setting should match input configuration"
  }

  assert {
    condition     = powerplatform_managed_environment.this.limit_sharing_mode == var.test_sharing_settings.limit_sharing_mode
    error_message = "Sharing mode should match input configuration"
  }

  assert {
    condition     = powerplatform_managed_environment.this.max_limit_user_sharing == var.test_sharing_settings.max_limit_user_sharing
    error_message = "User sharing limit should match input configuration"
  }

  assert {
    condition     = powerplatform_managed_environment.this.is_usage_insights_disabled == var.test_usage_insights_disabled
  ...
  assert {
    condition     = powerplatform_managed_environment.this.solution_checker_mode == var.test_solution_checker.mode
    error_message = "Solution checker mode should match input configuration"
  }

  assert {
    condition     = powerplatform_managed_environment.this.suppress_validation_emails == var.test_solution_checker.suppress_validation_emails
  ...
  assert {
    condition     = powerplatform_managed_environment.this.maker_onboarding_markdown == var.test_maker_onboarding.markdown_content
    error_message = "Maker onboarding markdown should match input configuration"
  }

  assert {
    condition     = powerplatform_managed_environment.this.maker_onboarding_url == var.test_maker_onboarding.learn_more_url
