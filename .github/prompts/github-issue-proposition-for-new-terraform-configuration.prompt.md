...
echo "ALLOWED FILES:"
echo "✅ NEW: configurations/{CONFIGURATION_NAME}/*"
echo "✅ UPDATE: CHANGELOG.md (add one entry only)"
echo ""
echo "🚫 FORBIDDEN FILES (DO NOT TOUCH):"
echo "❌ LICENSE"
echo "❌ README.md"  
echo "❌ config.env*"
echo "❌ .github/*"
echo "❌ scripts/*"
echo "❌ docs/*"
echo "❌ Any existing configurations/*"
echo "❌ Any other files"
```

## 🚨 CRITICAL IMPLEMENTATION RULES

### RULE 1: CREATE ONLY, DO NOT MODIFY
```bash
# ✅ ALLOWED: Create new configuration directory
mkdir -p configurations/{CONFIGURATION_NAME}

# ❌ FORBIDDEN: Modify any existing files except CHANGELOG.md
```

### RULE 2: TWO-FILE MODIFICATION LIMIT
**EXACTLY 2 types of file changes allowed:**
1. **NEW FILES**: `configurations/{CONFIGURATION_NAME}/*` (unlimited new files in this directory)
...
- [ ] Understood only 2 file types can be modified
...
**⚠️ SCOPE REMINDER**: Only modify configurations/{CONFIGURATION_NAME}/* and CHANGELOG.md
...
**⚠️ CRITICAL**: Follow the process described in `.github/prompts/init-terraform-configuration.prompt.md` exactly.
**⚠️ SCOPE**: Modify ONLY the new configuration directory and CHANGELOG.md
...
2. **Copy Template Directory** 
   ...
   # Validate scope compliance immediately
   git diff --name-only
   # Should only show configurations/{CONFIGURATION_NAME}/*
   ```
...
#### 📚 **Required Instructions Compliance**
```markdown
## 📚 Required Instructions Compliance
...
## 🔧 Classification-Specific Templates
...
- **Unambiguous scope restrictions** to prevent unauthorized modifications

This enhanced prompt creates actionable GitHub issues that eliminate ambiguity and ensure successful Terraform configuration implementation through automated agents while protecting repository integrity through clearly defined, unavoidable scope restrictions.