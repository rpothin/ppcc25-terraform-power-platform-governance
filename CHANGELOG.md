...
### Fixed
- **CONSISTENCY**: Pattern configuration `ptn-azure-vnet-extension` subnet naming consistency resolved across all VNet modules. Fixed private endpoint subnet naming inconsistency where mixed case ("Dev") was used instead of consistent lowercase ("dev") matching PowerPlatform subnets. **Root Cause**: Direct string interpolation instead of centralized CAF naming patterns with lowercase conversion. **Solution**: Implemented centralized `private_endpoint_subnet_name` in `environment_resource_names` using consistent `lower(replace(replace(env.suffix, " ", ""), "-", ""))` pattern matching PowerPlatform subnets. **Impact**: All private endpoint subnets now follow CAF naming conventions with proper case consistency across production primary, non-production primary, production failover, and non-production failover VNets.
- **CRITICAL**: Pattern configuration `ptn-azure-vnet-extension` Private DNS zones deployment issue resolved. DNS zones were not being deployed because the `private_dns_zones` variable defaults to empty set `[]` and the regional-examples.tfvars file didn't specify any zones. Fixed by adding 5 common Azure service DNS zones (Key Vault, Storage Blob/Files, Cosmos DB, SQL Database) to regional-examples.tfvars. **Root Cause**: `setproduct(environments, dns_zones)` with empty DNS zones creates zero combinations, preventing module instantiation. **Impact**: DNS zones will now deploy properly for private endpoint connectivity testing and demonstrations.
- **CRITICAL**: Pattern configuration `ptn-azure-vnet-extension` Phase 2 network security group rule errors resolved by correcting invalid Azure service tag. Fixed "SecurityRuleInvalidAddressPrefix" deployment error caused by using non-existent "PowerPlatform" service tag by replacing with valid "PowerPlatformInfra" service tag for Power Platform infrastructure connectivity. Error manifested during terraform apply as "Security rule has invalid Address prefix. Value provided: PowerPlatform" in NSG rule "AllowPowerPlatformInbound/Outbound". Solution maintains zero-trust networking functionality while ensuring Azure compliance with valid service tags. All 122 integration test assertions continue passing with corrected configuration. **Additional Issue**: Resource conflict errors indicate partial deployment state requiring manual intervention or Terraform import commands for affected resources (VNets, NSGs) to resolve state drift.released

### Fixed
- **CRITICAL**: Pattern configuration `ptn-azure-vnet-extension` Phase 2 network security group rule errors resolved by correcting invalid Azure service tag. Fixed "SecurityRuleInvalidAddressPrefix" deployment error caused by using non-existent "PowerPlatform" service tag by replacing with valid "PowerPlatformInfra" service tag for Power Platform infrastructure connectivity. Error manifested during terraform apply as "Security rule has invalid Address prefix. Value provided: PowerPlatform" in NSG rule "AllowPowerPlatformInbound/Outbound". Solution maintains zero-trust networking functionality while ensuring Azure compliance with valid service tags. All 124 integration test assertions continue passing with corrected configuration. **Additional Issue**: Resource conflict errors indicate partial deployment state requiring manual intervention or Terraform import commands for affected resources (VNets, NSGs) to resolve state drift.

### Enhanced  
- **ARCHITECTURE**: Pattern configuration `ptn-azure-vnet-extension` simplified NSG architecture following "Keep It Simple" principle from baseline instructions. Consolidated from 4 separate NSGs (2 per environment) to 2 unified NSGs (1 per environment) serving both PowerPlatform and PrivateEndpoint subnets. **Simplification Benefits**: Eliminated duplicate resources with identical security rules, reduced management overhead while maintaining exact same 5-rule zero-trust security posture, single source of truth for security configuration per environment. **Implementation**: Replaced `production_power_platform_nsgs`, `non_production_power_platform_nsgs`, `production_private_endpoint_nsgs`, `non_production_private_endpoint_nsgs` with unified `production_nsgs` and `non_production_nsgs` modules. **Test Coverage**: Updated integration tests from 124 to 122 assertions (488% over minimum), enhanced architecture documentation with NSG consolidation rationale. **Impact**: Demonstrates Infrastructure as Code best practices - choosing simple over complex while maintaining identical functionality and security.
- **SECURITY**: Pattern configuration `ptn-azure-vnet-extension` zero-trust networking rules optimized for true VNet injection principles. Removed unnecessary NSG rules that contradicted VNet injection security model: eliminated `AllowPowerPlatformInbound` (VNet injection eliminates need for external inbound traffic), removed `AllowAzureADOutbound` and `AllowStorageOutbound` (authentication and storage handled through private connectivity). **Refined Security Rules**: Now implements 5 focused rules - VNet inbound/outbound communication, PowerPlatform outbound connectivity only, Internet traffic denial inbound/outbound. **Architecture Principle**: VNet injection provides secure, private connection eliminating broad service tag allowances and external dependencies. **Test Coverage**: Updated 122 integration test assertions to validate refined 5-rule architecture with enhanced error messages explaining VNet injection benefits. **Impact**: Enforces true zero-trust by eliminating unnecessary external service dependencies while maintaining Power Platform functionality through private connectivity.

### Enhanced
- **MAJOR**: Pattern configuration `ptn-azure-vnet-extension` Phase 1 enhancements completed with private DNS zones and zero-trust networking capabilities. **Phase 1 Features**: Added flexible `private_dns_zones` variable (set of strings, max 10 zones) with comprehensive DNS zone name validation for on-demand Azure service connectivity, `enable_zero_trust_networking` boolean toggle (default: true) with conditional security rule application. **Zero-Trust Security Implementation**: 8 comprehensive NSG rules in locals with allow_vnet_inbound/outbound for VNet communication, allow_powerplatform_inbound/outbound for Power Platform service tag access, allow_azuread_outbound for Azure AD authentication, allow_storage_outbound for Azure Storage access, deny_internet_inbound/outbound for internet traffic blocking, proper priority ranges (100-130 for allow, 4000 for deny). **Enhanced Integration Testing**: Expanded from 69 to 92 total test assertions (368% over AVM minimum requirement), 23 new Phase 1 specific tests including phase1_dns_zones_validation for DNS zone scenarios, phase1_zero_trust_disabled_validation for development scenarios, comprehensive variable validation and security rule structure testing. **Phase 2 Structure Ready**: AVM module integration points identified for Azure/avm-res-network-privatednszone/azurerm and Azure/avm-res-network-networksecuritygroup/azurerm, Phase 1 compatible outputs prepared, conditional logic framework implemented and validated. **Technical Achievement**: All integration tests passing, comprehensive WHY-focused documentation, security by design with configurable development mode, flexible DNS zones supporting any Azure service private connectivity. **Next Phase**: Provider version conflict resolution required between child modules (~> 3.71) and AVM modules (~> 4.0) for Phase 2 completion.
- **MAJOR**: Pattern configuration `ptn-azure-vnet-extension` significantly enhanced with dual VNet enterprise architecture for business continuity and advanced governance. Major improvements include: **Dual VNet Architecture** supporting primary/failover regions (Canada Central/East) with non-overlapping IP ranges (10.100.x.x/10.101.x.x), dedicated private endpoint subnets for Azure service connectivity in both regions, comprehensive network validation with CIDR overlap detection and subnet containment verification. **Dynamic State Configuration** replacing static state_config variable with inherited backend configuration, automatic state key generation following pattern-based naming convention. **Enhanced Documentation Standards** with WHY-focused commenting aligned to terraform-iac.instructions.md, automated README generation via GitHub Actions workflow using _header.md and _footer.md templates. **Testing Infrastructure** with test_mode variable enabling mock data for comprehensive testing without backend dependencies, dual VNet validation in integration tests with 10+ network-specific assertions, enhanced edge case testing with small CIDR ranges. **Production Ready Features** including enterprise-grade network validation, business continuity regional placement strategy, comprehensive error handling with actionable messages, sensitive data protection in outputs, and maintained Phase 1 compliance with all original functionality preserved. This enhancement transforms the pattern from single VNet proof-of-concept to enterprise-ready dual region architecture suitable for production Power Platform governance scenarios.
...
### Added
- **NEW**: Resource configuration `res-enterprise-policy` for deploying Microsoft.PowerPlatform/enterprisePolicies using azapi provider to enable advanced Power Platform governance capabilities. Features comprehensive support for NetworkInjection (Azure VNet integration) and Encryption (customer-managed keys) policy types with: AVM-compliant child module architecture (no provider/backend blocks) for meta-argument compatibility (for_each, count, depends_on), comprehensive validation with strong typing and HEREDOC descriptions, dynamic configuration processing with type-specific property injection, anti-corruption layer outputs (enterprise_policy_id, enterprise_policy_system_id, policy_deployment_summary), managed identity integration with system-assigned identity, lifecycle management with governance-critical resource protection, comprehensive testing with 20+ assertions covering both policy types, example tfvars files for NetworkInjection and Encryption scenarios, and production-ready timeout configuration. Enables seamless integration with ptn-azure-vnet-extension pattern and Power Platform environment governance workflows.
- **DOCUMENTATION**: Simple "Known Limitations and Platform Constraints" documentation following baseline instruction principles of simplicity and clarity. Documents Power Platform teardown limitation with application admin resources, providing clear problem statement (exact error messages), root cause explanation (intentional platform security design), simple workaround (manual cleanup via Power Platform Admin Center), and framework for documenting future limitations. **ENHANCED** with duplicate detection complexity limitation based on GitHub issues #22 and #23, documenting fundamental Terraform architectural constraint where circular dependency between state file queries and resource creation makes true duplicate detection impossible within same configuration. Documents how attempted workarounds create complex, imperfect validation that provides false security while violating 200-line limits, obscuring core IaC concepts, and reducing educational effectiveness. Provides clear guidance on accepting Terraform's limitation and using proper alternatives (organizational policies, workspace isolation, external validation) for production environments. Focuses on essential information for PPCC25 demonstrations while maintaining educational value without unnecessary complexity.
