...
### Added
  - **NEW**: Resource configuration `res-environment-group-rule-set` for creating and managing Power Platform Environment Group Rule Sets to apply consistent governance policies across environments within groups at scale, enabling centralized administration and standardized compliance controls through Infrastructure as Code. Features include: AVM-compliant structure with res-* module patterns, comprehensive rule configuration (sharing controls, usage insights, maker welcome content, solution checker enforcement, backup retention, AI generated descriptions, AI generative settings), strong variable typing with GUID validation and rule-specific validation, lifecycle protection with ignore_changes for manual admin center modifications, anti-corruption layer outputs (environment_group_rule_set_id, environment_group_id, rule_set_configuration_summary), comprehensive 30+ test assertions covering plan/apply scenarios, OIDC authentication support, and integration readiness for environment group governance. Supports centralized governance, sharing control, solution quality enforcement, backup management, and AI governance across environment groups for consistent organizational policies and reduced manual configuration overhead.
